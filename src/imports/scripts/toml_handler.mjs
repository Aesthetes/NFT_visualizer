...
function extractObjectsFromTOML(field_name, toml_text) {
  ...
  for (let i = 0; i < array_of_lines.length; i++) {
    ...
    if (matches !== null && matches.length === 2) {
      //if the line defines an object name
      matching_object = matches[1] === field_name;

      if (extracting_object !== null) {
        //if an object was being extracted
    ...
    if (matching_object) {
      //if you are reading the fields of an object
      if (extracting_object === null) {
        //if the object is not already being extracted
        extracting_object = {}; //create the new object
      }

      let _obj_field_regex =
        /^(?<fieldName>[^#"=]+) = (?<fieldContent>[^#"=]+|\"[^#"=]+\")$/g;
